#********************************************************
#*　　　　TYPE B
#* n225d50bファイル TYPE B　neuralnet()で高値予測
#*
#*
#******************************************************** 
...
formula <- n225h ~ dow + nas + sp + usdidx + crb + usdjp + nkcmd
...
pred_actual <- denormalize(pred, data$n225h[-index])  # テストデータに対応



# ⑦ 精度評価（MSEなど）

mse <- mean((pred_actual - data$n225h[-index])^2)

print(paste("TYPE-Bﾌｧｲﾙ:平均二乗誤差 (MSE):", round(mse, 2)))

print(paste("TYPE-Bﾌｧｲﾙ:平均的な高値の予測誤差(RMSE円) :", round(sqrt(mse), 2)))

# [1] "平均二乗誤差 (MSE): 2264799.12"
...
# ✅ 合格かどうか判断するための視点
# 　| 判断基準 　　　　　  | 内容 | 
#   | 📉 MSEが極端に大きい | → 学習不足、過学習、または説明変数の情報量不足 | 
#   | 🎯 誤差が数百円程度  | → 株価水準に対して “悪くない” が、用途次第で改善余地あり | 
#   | 🔍 可視化してみる    | → 実測 vs 予測 のグラフで傾向が一致しているか確認 | 
#   | 🔧 チューニング      | → 隠れ層構造や正規化方法を変えると精度が向上することも | 
#   

# このﾓﾃﾞﾙをつかって説明変数が
# 調べたい日のﾃﾞｰﾀの場合、
# 目的変数n225hの予測値を出しそれを示す手順を教えてください。
...
pred_value <- pred_result * (max(data$n225h) - min(data$n225h)) + min(data$n225h)

print(paste("TYPE-Bﾌｧｲﾙで予測された n225h の高値:", round(pred_value, 2)))

# "予測された n225h の値: 41539.28" とでました。
# "予測された 2025/7/25のn225h の値: 
# ﾆｭｰﾗﾙﾈｯﾄﾜｰｸ予測：41539.28"(-234)
# macdの予想は42,437(-664)
# 結果は41773
